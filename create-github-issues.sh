...
# Script to create GitHub issues for Calendar Modular project
# Run this with: bash create-github-issues.sh

echo "Creating Phase 1 GitHub Issues..."

# Phase 1.1: Project Initialization
gh issue create --title "Phase 1.1: Project Initialization" --body "**Goal:** Set up the basic React + TypeScript project structure with Vite.
...
gh issue create --title "Phase 1.2: Install Core Dependencies" --body "**Goal:** Add all the libraries needed for routing, UI, calendar display, and drag-and-drop.
...
# Phase 1.3: Set Up Shadcn/ui
gh issue create --title "Phase 1.3: Set Up Shadcn/ui (UI Component Library)" --body "**Goal:** Install a modern component library for buttons, forms, modals, etc.
...
# Phase 1.4: Create Supabase Project
gh issue create --title "Phase 1.4: Create Supabase Project and Configure Authentication" --body "**Goal:** Set up backend database and user authentication system.
...
# Phase 1.5: Build Login and Signup Pages
gh issue create --title "Phase 1.5: Build Login and Signup Pages" --body "**Goal:** Create pages where users can create accounts and log in.
...
# Phase 1.6: Set Up Routing
gh issue create --title "Phase 1.6: Set Up Routing and Protected Routes" --body "**Goal:** Add navigation between pages and prevent access to calendar without logging in.
...
# Phase 1.7: Create Database Schema
gh issue create --title "Phase 1.7: Create Database Schema for Events and Classes" --body "**Goal:** Create database tables to store calendar events AND extracted class information from PDFs.
...
# Phase 1.8: Set Up Google Gemini
gh issue create --title "Phase 1.8: Set Up Google Gemini for PDF Parsing (PRIMARY FEATURE)" --body "**Goal:** Integrate with Google Gemini AI to extract class information from uploaded PDF schedules.
...
# Phase 1.9: Build PDF Upload Component
gh issue create --title "Phase 1.9: Build PDF Upload Component" --body "**Goal:** Create file upload interface that sends PDFs to Gemini for class extraction.
...
# Phase 1.10: Build Class Catalog Browser
gh issue create --title "Phase 1.10: Build Class Catalog Browser" --body "**Goal:** Create sidebar showing all extracted classes with hide/unhide toggles.
...
# Phase 1.11: Build Calendar View
gh issue create --title "Phase 1.11: Build Calendar View with Fixed and Flexible Blocks" --body "**Goal:** Create a calendar displaying two types of blocks—fixed (PDF classes) and flexible (personal events)—with different behaviors.

## Design Reference
![Calendar Design Mockup](https://github.com/sheethalmjacob/calendar-modular/blob/main/docs/design/FigmaCalendar.png)
...
# Phase 1.12: Add Drag-and-Drop
gh issue create --title "Phase 1.12: Add Drag-and-Drop Event Manipulation" --body "**Goal:** Enable dragging events to different times and resizing them.

## Update Calendar Component
Update \`src/pages/Calendar.tsx\` (see PLAN.md Step 1.9 for code):
...
# Phase 1.13: Import from Google Calendar
gh issue create --title "Phase 1.13: Add Calendar Import from Google Calendar" --body "**Goal:** Let users import events from their Google Calendar as a one-time copy.
...
See PLAN.md Step 1.10 for complete code.
...
# Phase 1.14: Import from Outlook
gh issue create --title "Phase 1.14: Add Calendar Import from Microsoft Outlook" --body "**Goal:** Add import functionality for Outlook/Microsoft 365 calendars.
...
See PLAN.md Step 1.11 for complete code.
...
# Phase 1.15: Import from Apple Calendar
gh issue create --title "Phase 1.15: Add Calendar Import from Apple Calendar (iCal)" --body "**Goal:** Add file upload for .ics (iCalendar) files exported from Apple Calendar.
...
See PLAN.md Step 1.12 for complete code.
...
# Phase 1.16: Deploy to Vercel
gh issue create --title "Phase 1.16: Deploy to Vercel" --body "**Goal:** Make your app live on the internet.
...
echo ""
echo "To create Phase 2, 3, and 4 issues, you can extend this script or create them manually."
